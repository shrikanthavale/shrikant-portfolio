...
export default function SubpageTopBar({
  ...
  const leftContent = leftHref ? (
    <Link href={leftHref} className="text-link-subtle text-sm font-medium text-sky-700 hover:text-sky-600 dark:text-sky-300 dark:hover:text-sky-200">
      {leftLabel}
    </Link>
  ) : (
  ...
  return (
    <section className="border-b border-slate-200 bg-white/90 dark:border-slate-800 dark:bg-slate-950/90">
      <div className={`mx-auto flex ${maxWidthClass} items-center justify-between px-6 py-4`}>
        {leftContent}
        <Link href={rightHref} className="text-link-subtle text-sm font-medium text-slate-500 hover:text-sky-600 dark:text-slate-400 dark:hover:text-sky-200">
          {rightLabel}
        </Link>
      </div>
    </section>
